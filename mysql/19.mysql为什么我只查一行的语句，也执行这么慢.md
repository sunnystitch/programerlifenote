...
![图 1 查询长时间不返回](./mysql19-1.png)

<center>图 1 查询长时间不返回</center>
...
![图 2 Waiting for table metadata lock 状态示意图](./mysql19-2.png)
...
![图 3 MySQL 5.7 中 Waiting for table metadata lock 的复现步骤](./mysql19-3.png)

<center>图 3 MySQL 5.7 中 Waiting for table metadata lock 的复现步骤</center>
...
![图 4 查获加表锁的线程 id](./mysql19-4.png)

<center>图 4 查获加表锁的线程 id</center>
...
![图 5 Waiting for table flush 状态示意图](./mysql19-5.png)

<center>图 5 Waiting for table flush 状态示意图</center>
...
![图 6 Waiting for table flush 的复现步骤](./mysql19-6.png)

<center>图 6 Waiting for table flush 的复现步骤</center>
...
![图 7 Waiting for table flush 的 show processlist 结果](./mysql19-7.png)

<center>图 7 Waiting for table flush 的 show processlist 结果</center>
...
复现步骤和现场如下：
![图 8 行锁复现](./mysql19-8.png)

<center>图 8 行锁复现</center>

![图 9 行锁 show processlist 现场](./mysql19-9.png)

<center>图 9 行锁 show processlist 现场</center>
...
![图 10 通过 sys.innodb_lock_waits 查行锁](./mysql19-10.png)

<center>图 10 通过 sys.innodb_lock_waits 查行锁</center>
...
![图 11 全表扫描 5 万行的 slow log](./mysql19-11.png)

<center>图 11 全表扫描 5 万行的 slow log</center>
...
![图 16 id=1 的数据状态](./mysql19-16.png)

<center>图 16 id=1 的数据状态</center>
...
加的锁又是什么时候释放呢？你可以把你的观点和验证方法写在留言区里，我会在下一篇文章的末尾给出我的参考答案。感谢你的收听，也欢迎你把这篇文章分享给更多的朋友一起阅读。

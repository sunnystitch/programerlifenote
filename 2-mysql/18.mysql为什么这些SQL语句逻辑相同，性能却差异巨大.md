...
![图 1 t_modified 索引示意图](./mysql18-1.png)
...
![图 2 explain 结果](./mysql18-2.png)

<center>图 2 explain 结果</center>

ey="t_modified"表示的是，使用了 t_modified 这个索引；我在测试表数据中插入了 10 万行数据，rows=100335，说明这条语句扫描了整个索引的所有值；Extra 字段的 Using index，表示的是使用了覆盖索引。
...
![图 3 MySQL 中字符串和数字转换的效果示意图](./mysql18-3.png)
...
<center>图 4 语句 Q1 的 explain 结果</center>
...
![图 5 语句 Q1 的执行过程](./mysql18-5.png)

<center>图 5 语句 Q1 的执行过程</center>
...
- 如果能够修改字段的字符集的话，是最好不过了。但如果数据量比较大， 或者业务上暂时不能做这个 DDL 的话，那就只能采用修改 SQL 语句的方法了。
  ...
  ![图 7 SQL 语句优化后的 explain 结果](./mysql18-7.png)

  <center>图 7 SQL 语句优化后的 explain 结果</center>
